...
const container = document.getElementById('interactive-background') || document.getElementById('container');
if (!container) {
  throw new Error('Interactive background container not found');
}

const getViewportWidth = () => Math.max(320, container.clientWidth || window.innerWidth);
...
window.addEventListener('resize', () => {
  if (c && c.width) {
    sizeCanvas();
    CONFIG.awidth = getViewportWidth();
    CONFIG.aheight = getViewportHeight();
    CONFIG.cellWidth = CONFIG.awidth / (CONFIG.gridW - 1);
    CONFIG.cellHeight = CONFIG.aheight / (CONFIG.gridH - 1);
  }
})
